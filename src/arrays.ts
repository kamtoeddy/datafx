import { DeepKeyOf, DeepValueOf, ObjectKey, ObjectType } from './types';
...
function countBy<T, OutputKeys extends ObjectKey = string>(
  array: T[],
  determinant?: Counter<T> | DeepKeyOf<T>
) {
...
type FilterAsObject<T> = Partial<{ [K in DeepKeyOf<T>]: DeepValueOf<T, K> }>;
type FilterOptions = { exclude?: boolean };

const defaultOptions: FilterOptions = { exclude: false };

function filterBy<T, K extends DeepKeyOf<T> = DeepKeyOf<T>>(
  array: T[],
  determinant: Filter<T> | FilterAsObject<T> | [K, DeepValueOf<T, K>],
...
type FindAsObject<T> = Partial<{ [K in DeepKeyOf<T>]: DeepValueOf<T, K> }>;
type FindByOptions = { fromBack?: boolean };

function findBy<T, K extends DeepKeyOf<T> = DeepKeyOf<T>>(
  array: T[],
  determinant: Finder<T> | FindAsObject<T> | [K, DeepValueOf<T, K>],
  ...
  if (Array.isArray(determinant))
    return _array.find((dt) => {
      const [key, value] = determinant;
      const dt_val = getDeepValue(dt as any, key);
...
function getListOf<T, K extends DeepKeyOf<T> = DeepKeyOf<T>>(
  array: T[],
  key: K,
...
function getUniqueBy<T, K extends DeepKeyOf<T> = DeepKeyOf<T>>(
  array: T[],
  key?: K,
...
type GroupedMap<OutputKeys extends ObjectKey, T> = Record<OutputKeys, T[]>;

function groupBy<T, OutputKeys extends ObjectKey = string>(
  array: T[],
  determinant: Grouper<T> | DeepKeyOf<T>
) {
  if (!array) return {} as GroupedMap<OutputKeys, T>;

  const asFx = typeof determinant === 'function';

  return array.reduce((prev, next) => {
    const key = asFx
      ? (determinant(next) as DeepKeyOf<T>)
      : (getDeepValue(
          next as ObjectType,
...
function sortBy<T>(
  array: T[],
  determinant?: Sorter<T> | DeepKeyOf<T>,
  order: SortOrder = 'asc'
) {
